...
def validate_password(field):
    ...
    if not bool(re.match(pattern, field)):
        ...
        raise ValidationError(error_massages[0][language], code=error_massages[0][language])
    if not 8 <= len(field) <= 16:
        print(error_massages[1][language])
        raise ValidationError(error_massages[1][language], code=error_massages[1][language])